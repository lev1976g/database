#!/bin/bash
# WhyDB Linux Tool v2.0 by Latheesan - Admin of Nemesis-WarCraft.Com
############################################################################
#
#  Tool Configuration
#
#  user - MYSQL username
#  pass - MYSQL password
#  ldb - your logon database
#  cdb - your char database
...
logo() {
	clear
	echo
	echo " __          ___           _____  ____   "
	echo " \ \        / / |         |  __ \|  _ \  "
	echo "  \ \  /\  / /| |__  _   _| |  | | |_) | "
	echo "   \ \/  \/ / | '_ \| | | | |  | |  _ <  "
	echo "    \  /\  /  | | | | |_| | |__| | |_) | "
	echo "     \/  \/   |_| |_|\__, |_____/|____/  "
	echo "                      __/ |              "
	echo "                     |___/               "
	echo
...
until [ "${option}" = "x" ]; do
	logo
	echo " i - Install Clean WhyDB World Database"
	echo " u - Update WhyDB World Database"
	echo " b - Backup Logon and Char Database"
	echo " r - Restore Logon and Char Database Backup"
	echo " x - Exit Tool"
	echo
		...
		if [ "${available_changesets}" != "${uppath}/changeset_*.sql" ]; then
			...
			if [ "${index}" != "x" ]; then
				...
				if [ ! -f "${update}" ]; then
					...
					echo
					echo " [Updating WhyDB World Database] Finished..."
				fi
			fi
	...
	echo " Thanks for using WhyDB Linux Tool v2.0 by Latheesan!"
	echo "  Updated 21-06-2010 by WhyDB"
done
